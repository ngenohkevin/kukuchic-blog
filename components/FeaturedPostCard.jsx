...
const FeaturedPostCard = ({post}) => (
    <div className="relative h-72">
        ...
        <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full">
            ...
            {post?.authors?.map(p => (
                <div key={p?.name} className="flex items-center absolute bottom-5 w-full justify-center">
                    <Image
                        unoptimized
                        alt={p?.name}
                        height="30px"
                        ...
                        src={p?.photo?.url}
                    />
                    <p className="inline align-middle text-white text-shadow ml-2 font-medium">{p?.name}</p>
                </div>
            ))}
        </div>
